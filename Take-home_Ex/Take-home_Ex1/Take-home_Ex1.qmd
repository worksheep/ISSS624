...
The weekday morning peak, we see congregation of high-high cluster in major transport nodes like MRT / Bus Interchange nearer to the residential area.

Whereas the low-low clusters are in the fringe area especially in the Tuas region, where there are little residential area
```{r}
tm_shape(sgp_simple)+
  tm_polygons(col="white", 
              alpha = 0.6,
...
The weekday evening peak is somewhat surprising, as i would have expected some high-high cluster in the CBD. 
